...
3. **Use Nix flakes and justfile** for all infrastructure and tooling. No ad-hoc install instructions.
4. **Capture bugs and new feature ideas as backlog tasks** (`backlog task create ...`). Proactively add backlog tasks when you spot work that needs doing. **Backlog tasks must be implemented by fresh sub-agents** (use the Agent tool), not inline in the current conversation — this keeps context clean and tasks atomic.
