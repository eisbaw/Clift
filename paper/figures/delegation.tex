...
\begin{tikzpicture}[
  ...
  % Backlog
  \node[box, fill=yellow!15, minimum width=2.2cm] (backlog) at (3.5, 3.0)
    {Backlog\\[-2pt]\scriptsize 230+ tasks};

  % Memory
  \node[box, fill=yellow!15, minimum width=2.2cm] (memory) at (3.5, 1.8)
  ...
  \draw[arrow, color=green!50!black] (kernel.south) -- ++(0,-0.4) -| (-4.5,3.0)
    ...
    -- (primary.west);
\end{tikzpicture}
\caption{Layered delegation architecture. The human sets goals;
the primary agent spawns specialists; the Lean~4 kernel provides
ground truth.}
