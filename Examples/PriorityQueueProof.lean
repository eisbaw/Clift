...
pq_insert calls pq_bubble_up; pq_extract_min calls pq_bubble_down.
Both callees are accessed via L1.call through the L1ProcEnv generated by clift.

## What's proven here

- L1ProcEnv lookup: the call to "pq_bubble_up" / "pq_bubble_down" in the
  generated L1 body resolves to the correct L1 callee body.
- Caller proof structure: the outer catch/condition/seq decomposition is
  partially verified using L1 Hoare rules.

## What blocks full sorry elimination

Three pieces of infrastructure are needed:

1. **Non-aliasing (ptrDisjoint)**: After writing data[size] (heapUpdate at a
   UInt32 pointer), we read pq.size again (hVal at a C_pqueue pointer).
   To prove hVal is unchanged, we need `hVal_heapUpdate_disjoint` which
   requires `ptrDisjoint (Ptr.elemOffset data size) pq`. The precondition
   must include this non-aliasing assertion. Without it, the invariant
   `(hVal heap pq).size < (hVal heap pq).capacity` cannot be threaded
   through the heapUpdate for data[size].

2. **Callee specs (pq_bubble_up, pq_bubble_down)**: Each callee contains a
   while loop calling pq_swap via dynCom. Proving the callee spec requires:
   - A FuncSpec for pq_swap (3 guard+modify steps, no calls)
   - A while-loop invariant: i decreases (parent = (i-1)/2 < i for bubble_up),
     all accessed array indices are valid, heapPtrValid for pq is preserved
   - dynCom/call composition for pq_swap within the loop body

3. **dynCom composition in the caller**: The save/setup/call/restore pattern
   in the caller must be threaded through L1_hoare_dynCom. After restore,
   locals come from saved state but globals from after the callee. The
   callee postcondition must be strong enough to establish the caller's
   intermediate invariant on the restored state.

These three blockers are interdependent: the caller proof needs callee specs,
the callee specs need pq_swap specs, and both need non-aliasing. -/

/-- The L1ProcEnv embedded in l1_pq_insert_body by clift (for bubble_up call).
    This matches the environment generated by clift_l1 for functions that call
    pq_bubble_up. We extract it as a named definition so L1ProcEnv lookups
    can be proved by simp with L1 bodies marked irreducible. -/
...
-- L1ProcEnv lookup: "pq_bubble_up" resolves to l1_pq_bubble_up_body.
-- Proved by simp on the insert chain with L1 bodies irreducible
-- (prevents kernel from recursing into the body definitions).
...
-- L1.call resolves: call via insert_l1Γ equals the callee body directly.
attribute [local irreducible]
  PriorityQueue.l1_pq_init_body PriorityQueue.l1_pq_swap_body
  PriorityQueue.l1_pq_bubble_up_body PriorityQueue.l1_pq_bubble_down_body
  PriorityQueue.l1_pq_insert_body PriorityQueue.l1_pq_extract_min_body
...
/-- The L1ProcEnv embedded in l1_pq_extract_min_body (for bubble_down call). -/
...
-- L1ProcEnv lookup: "pq_bubble_down" resolves to l1_pq_bubble_down_body.
attribute [local irreducible]
  PriorityQueue.l1_pq_init_body PriorityQueue.l1_pq_swap_body
  PriorityQueue.l1_pq_bubble_up_body PriorityQueue.l1_pq_bubble_down_body
  PriorityQueue.l1_pq_insert_body PriorityQueue.l1_pq_extract_min_body
...
-- L1.call resolves: call via extract_min_l1Γ equals the callee body directly.
attribute [local irreducible]
  PriorityQueue.l1_pq_init_body PriorityQueue.l1_pq_swap_body
  PriorityQueue.l1_pq_bubble_up_body PriorityQueue.l1_pq_bubble_down_body
  PriorityQueue.l1_pq_insert_body PriorityQueue.l1_pq_extract_min_body
...
The sorry in these proofs is due to the three blockers documented above
(non-aliasing, callee specs, dynCom composition). The infrastructure for
inter-procedural call resolution (L1ProcEnv lookups, call resolution lemmas)
is in place and proven above.

The outer proof structure (catch + condition dispatch) has been verified
to type-check with the Hoare rules; the remaining difficulty is in
threading the heap invariant through the sequential steps that involve
heapUpdate followed by hVal at a different pointer.

To complete these proofs, add to the precondition:
  ∀ i, i < capacity.toNat → ptrDisjoint (Ptr.elemOffset data i) pq
Then hVal_heapUpdate_disjoint can be used to show that writing data[i]
does not change (hVal heap pq), preserving the size < capacity invariant
through each step. -/

-- SORRY: requires non-aliasing (ptrDisjoint data[i] pq) + callee specs
-- for pq_bubble_up + dynCom composition. See documentation above.
theorem pq_insert_correct :
    pq_insert_spec.satisfiedBy PriorityQueue.l1_pq_insert_body := by
  sorry

-- SORRY: requires non-aliasing (ptrDisjoint data[i] pq, ptrDisjoint out pq)
-- + callee specs for pq_bubble_down + dynCom composition. See above.
