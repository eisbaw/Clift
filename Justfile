...
# Proof integrity audit: detect circular specs, hand-written intermediates, wrong targets
audit:
    #!/usr/bin/env bash
    set -euo pipefail
    ISSUES=0
    echo "=== Clift Proof Integrity Audit ==="

    # 1. Hand-written L1 bodies (should be auto-generated by clift)
    echo ""
    echo "--- Hand-written L1 bodies (should use clift auto-generation) ---"
    HW=$(grep -rn "^def l1_\|^noncomputable def l1_\|^private.*def l1_" \
      --include="*.lean" Examples/ | grep -v cruft | grep -v "let l1_\|-- l1_" | wc -l || true)
    if [ "$HW" -gt 0 ]; then
      echo "  WARN: $HW hand-written L1 body definitions in Examples/"
      echo "  These should use 'clift Module' auto-generation instead."
      grep -rn "^def l1_\|^noncomputable def l1_\|^private.*def l1_" \
        --include="*.lean" Examples/ | grep -v cruft | grep -v "let l1_\|-- l1_"
      ISSUES=$((ISSUES + HW))
    else
      echo "  OK: No hand-written L1 bodies"
    fi

    # 2. Proofs against manual/fused bodies (not the generated Module.l1_*_body)
    echo ""
    echo "--- Proofs against wrong target (manual/fused bodies) ---"
    FUSED=$(grep -rn "satisfiedBy.*l1_.*_manual\|satisfiedBy.*l1_.*_fused\|satisfiedBy.*l1_.*_local" \
      --include="*.lean" Examples/ | grep -v cruft | grep -v "^.*:.*--" | wc -l || true)
    if [ "$FUSED" -gt 0 ]; then
      echo "  WARN: $FUSED proofs target manual/fused bodies instead of generated ones"
      grep -rn "satisfiedBy.*l1_.*_manual\|satisfiedBy.*l1_.*_fused\|satisfiedBy.*l1_.*_local" \
        --include="*.lean" Examples/ | grep -v cruft | grep -v "^.*:.*--"
      ISSUES=$((ISSUES + FUSED))
    else
      echo "  OK: All proofs target generated bodies"
    fi

    # 3. Tautological postconditions (validHoare_weaken_trivial_post usage)
    echo ""
    echo "--- Tautological postcondition weakening ---"
    WEAKEN=$(grep -rn "validHoare_weaken_trivial_post" Examples/ --include="*.lean" \
      | grep -v cruft | grep -v "^.*:.*--" \
      | grep -v "private theorem validHoare_weaken_trivial_post" \
      | grep -c "validHoare_weaken_trivial_post" || true)
    if [ "$WEAKEN" -gt 0 ]; then
      echo "  WARN: $WEAKEN proofs use validHoare_weaken_trivial_post"
      grep -rn "validHoare_weaken_trivial_post" Examples/ --include="*.lean" \
        | grep -v cruft | grep -v "^.*:.*--" \
        | grep -v "private theorem validHoare_weaken_trivial_post"
      ISSUES=$((ISSUES + WEAKEN))
    else
      echo "  OK: No trivial postcondition weakening"
    fi

    # 4. Circular definitions (spec result set defined as the property to prove)
    echo ""
    echo "--- Potential circular specs (results defined as property) ---"
    CIRC=$(grep -rn "results := fun.*∃.*∧.*=" --include="*.lean" Examples/ \
      | grep -v cruft | grep -v "^.*:.*--" | wc -l || true)
    if [ "$CIRC" -gt 0 ]; then
      echo "  WARN: $CIRC definitions embed spec in result set (potentially circular)"
      grep -rn "results := fun.*∃.*∧.*=" --include="*.lean" Examples/ \
        | grep -v cruft | grep -v "^.*:.*--"
      ISSUES=$((ISSUES + CIRC))
    else
    ...
    LIB_SORRY=$(grep -rn "^\s*sorry" Clift/ --include="*.lean" | grep -c "sorry" || true)
    if [ "$LIB_SORRY" -gt 0 ]; then
      echo "  FAIL: $LIB_SORRY sorry in Clift/"
      grep -rn "^\s*sorry" Clift/ --include="*.lean"
      ISSUES=$((ISSUES + LIB_SORRY))
    else
      echo "  OK: Core library sorry-free"
    ...
    SORRY=$(grep -rn '; sorry\|by sorry\|^\s\+sorry' Examples/ --include="*.lean" \
      | grep -v cruft | wc -l || true)
    if [ "$SORRY" -gt 0 ]; then
      echo "  INFO: $SORRY sorry remaining in Examples/"
      ISSUES=$((ISSUES + SORRY))
    else
      echo "  OK: Zero sorry in Examples/"
    fi
    ...
    echo ""
    echo "=== Audit: $ISSUES issues found ==="
    if [ "$ISSUES" -gt 0 ]; then exit 1; fi
